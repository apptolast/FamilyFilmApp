...
//        field: MutableStateFlow<Result<FirebaseUser?>> = MutableStateFlow(Result.success(null))
    ...
    override fun login(email: String, password: String): Flow<Result<FirebaseUser?>> =
        callbackFlow {
            ...
            val success: (AuthResult) -> Unit = { launch { send(Result.success(it.user)) } }
            val failure: (Throwable) -> Unit = { launch { send(Result.failure(it)) } }
                    ...
                    if (task.isSuccessful) {
                        ...
                        user?.sendEmailVerification()?.addOnCompleteListener { task ->
                            if (task.isSuccessful) {
                                ...
                                launch { send(Result.success(user)) }
                                launch { send(Result.failure(Throwable(message = "Verification email sent"))) }
                            } else {
                                // Handle the error
